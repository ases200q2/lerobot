...
def evaluate_policy_in_simulation(policy, device, num_episodes=5):
    ...
    print("  🎲 Random cube positioning: ENABLED")
    ...
    results = {"episodes": [], "success_rate": 0.0, "average_episode_length": 0.0, "average_reward": 0.0}
        ...
        for episode in range(num_episodes):
            ...
            for step in range(100):  # Max 100 steps per episode
                ...
                try:
                    ...
                    if step % 10 == 0:
                        print(
                            f"      Step {step}: Action = [{action[0]:.3f}, {action[1]:.3f}, {action[2]:.3f}, {action[3]:.3f}]"
                        )
    ...
    results["average_episode_length"] = (
        total_episode_length / len(results["episodes"]) if results["episodes"] else 0.0
    )
